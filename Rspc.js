...
function calculateAndDisplay() {
    ...
    const profitPercent = parseFloat(document.getElementById('profitPercentage').value);
    const discountPercent = parseFloat(document.getElementById('discountPercentage').value);
    ...
    resultsDiv.innerHTML = `
        Profit Amount: ₹${profitAmount.toFixed(2)}<br>
        Profit Percentage over ${years} years: ${actualProfitPercent.toFixed(2)}%<br>
        Discount Amount: ₹${discountAmount.toFixed(2)}<br>
        Final Selling Price after Discount: ₹${finalSP.toFixed(2)}<br>
